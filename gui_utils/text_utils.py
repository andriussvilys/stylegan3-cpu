...
def get_default_font():
    url = 'https://fonts.gstatic.com/s/opensans/v17/mem8YaGs126MiZpBA-U1UpcaXcl0Aw.ttf'  # Open Sans regular
...
def get_array(string, *, dropshadow_radius: int = None, **kwargs):
    if dropshadow_radius is not None:
        ...
        return _get_array_priv(string, dropshadow_radius=dropshadow_radius, offset_x=offset_x, offset_y=offset_y,
                               **kwargs)
    else:
...
def _get_array_priv(
    ...
    while True:
        if dropshadow_radius is not None:
            ...
            array = _get_array_impl_dropshadow(string, size=cur_size, radius=dropshadow_radius, offset_x=offset_x,
                                               offset_y=offset_y, **kwargs)
        else:
            array = _get_array_impl(string, size=cur_size, **kwargs)
        height, width, _ = array.shape
        if (max_width is None or width <= max_width) and (max_height is None or height <= max_height) or (
                cur_size <= min_size):
            break
...
@functools.lru_cache(maxsize=10000)
def _get_array_impl(string, *, font=None, size=32, outline=0, outline_pad=3, outline_coef=3, outline_exp=2,
                    line_pad: int = None):
...
@functools.lru_cache(maxsize=10000)
def _get_array_impl_dropshadow(string, *, font=None, size=32, radius: int, offset_x: int, offset_y: int,
                               line_pad: int = None, **kwargs):
